    ...
    try {
        ...
        if (!subscription) {
            ...
            const suscripcionData: SuscripcionData = {
                subscription: {
                    ...
                    stripe_customer_id: '',
                    plan_id: plan.id,
                    status: studio.subscription_status as 'TRIAL' | 'ACTIVE' | 'CANCELLED' | 'PAUSED' | 'EXPIRED' | 'UNLIMITED',
                    current_period_start: studio.subscription_start || new Date(),
                    current_period_end: studio.subscription_end || new Date(), // Trial: días restantes se calculan desde subscription_end
        ...
        const suscripcionData: SuscripcionData = {
            subscription: {
                ...
                status: subscriptionWithRelations.status as 'TRIAL' | 'ACTIVE' | 'CANCELLED' | 'PAUSED' | 'EXPIRED' | 'UNLIMITED',
