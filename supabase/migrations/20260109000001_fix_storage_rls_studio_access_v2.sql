-- ============================================
-- FIX: Storage RLS Policies - Verificar acceso al studio específico (V2)
-- ============================================
-- Actualiza las políticas RLS de Storage para verificar que el usuario
-- tenga acceso al studio específico del path usando user_studio_roles O studio_user_profiles
...
-- Verifica acceso a través de user_studio_roles O studio_user_profiles
...
WITH CHECK (
  ...
  -- Usa user_studio_roles (múltiples studios) O studio_user_profiles (studio único)
  AND EXISTS (
    SELECT 1 FROM studios s
    WHERE s.slug = (storage.foldername(name))[2]::text
    AND (
      -- Opción 1: Usuario tiene acceso a través de user_studio_roles
      s.id IN (
      ...
      -- Opción 2: Usuario tiene acceso a través de studio_user_profiles (fallback)
      OR s.id IN (
        SELECT studio_id 
